...
12. If a completed/disconnected Mango call callback has no `recording_id`, including Mango summary callbacks that expose completion through `end_time`, CRM can infer candidate recording id from decoded `entry_id` as `1:<accountId>:<entryId>:0`, derive account id from explicit recording config, `call_id`, or the recording URL template, and persist `recordingUrl` only after the server-side Mango recording probe confirms playable audio.
