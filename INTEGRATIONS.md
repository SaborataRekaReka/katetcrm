...
12. If a completed/disconnected Mango call callback has no `recording_id`, CRM can infer candidate recording id from decoded `entry_id` as `1:<accountId>:<entryId>:0`, derive account id from config or `call_id`, and persist `recordingUrl` only after the server-side Mango recording probe confirms playable audio.
