...
11. Mango completed/disconnected call callbacks without `recording_id` may still produce `telephony.recordingUrl` by deriving candidate recording id from decoded `entry_id` and account id from config or `call_id`; CRM persists that inferred URL only after server-side Mango recording availability validation succeeds.
