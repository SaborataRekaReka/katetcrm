...
- `GET /api/v1/integrations/mango/recording-proxy?url=<mango_playback_url>`
...
7. Mango call-routing settings are admin-only, stored in `SystemConfig`, and map internal Mango extensions to active CRM managers for inbound call assignment. Mango lifecycle callbacks with `seq` are idempotent per lifecycle event under the same correlation id. If no extension/fallback matches on an inbound callback, ingest assigns from the active Mango routing manager pool; if no pool exists, it leaves the Lead unassigned instead of defaulting to the system admin actor.
...
11. Mango completed/disconnected call callbacks without `recording_id`, including summary callbacks that expose completion through `end_time`, may still produce `telephony.recordingUrl` by deriving candidate recording id from decoded `entry_id` and account id from explicit recording config, `call_id`, the recording URL template, or related lifecycle events with the same `entry_id`; CRM persists that inferred URL only after server-side Mango recording availability validation succeeds.
12. Once a Mango recording URL is verified for a call group, CRM may backfill existing Mango activity rows for the same `entry_id`/correlation group so previously logged lifecycle activity on duplicate/orphan leads exposes the same playable recording.
