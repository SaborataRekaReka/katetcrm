...
7. Mango call-routing settings are admin-only, stored in `SystemConfig`, and map internal Mango extensions to active CRM managers for inbound call assignment. If no rule/fallback matches, integration ingest leaves the Lead unassigned instead of defaulting to the system admin actor.
